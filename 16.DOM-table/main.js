...
// )
// })
let userAscending = document.querySelector(".ascending");
let userDescending = document.querySelector(".descending");
let userName = document.querySelector(".name");
///////////////////
userAscending.addEventListener("click", function () {
  let sortAsc = users.sort((a, b) => a.id - b.id);
  drawTable(sortAsc);
});
userDescending.addEventListener("click", function () {
  let sortDesc = users.sort((a, b) => b.id - a.id);
  drawTable(sortDesc);
});
////
...
search.addEventListener("keyup", function (event) {
  let filtered = users.filter((item) =>
    (item.first_name && item.email && item.gender && item.last_name)
      .toLocaleLowerCase()
...
let ascBtn = document.querySelector("click", function (event) {
  event.target;
  asc.innerText = "DESC";
});
